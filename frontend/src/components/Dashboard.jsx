    ...
    return (
        <div className="space-y-6 max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-white p-6 rounded-2xl shadow border border-gray-100 flex items-center shadow-md hover:shadow-lg transition-shadow">
                    <div className="w-14 h-14 rounded-full bg-blue-50 flex items-center justify-center mr-4">
                        <TrendingUp className="w-7 h-7 text-blue-600" />
                    </div>
                    <div>
                        <p className="text-sm font-medium text-gray-500 mb-1">Total Revenue (7 Days)</p>
                ...
                <div className="bg-white p-6 rounded-2xl shadow border border-gray-100 flex items-center shadow-md hover:shadow-lg transition-shadow">
                    ...
                    <div>
                        <p className="text-sm font-medium text-gray-500 mb-1">Sales Entries</p>
                        <p className="text-2xl font-bold text-gray-900">{analytics.sales.length}</p>
                    </div>
                </div>
                ...
                <div className="bg-white p-6 rounded-2xl shadow border border-gray-100 flex items-center shadow-md hover:shadow-lg transition-shadow">
                    ...
                    <div>
                        ...
                        <p className="text-2xl font-bold text-gray-900">{lowStockCount} items</p>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-8 shadow-sm">
                ...
                <div className="bg-white p-6 rounded-2xl shadow border border-gray-100 flex flex-col h-[400px]">
                    <h3 className="text-lg font-semibold text-gray-800 mb-4 flex items-center pb-4 border-b">
                        <AlertCircle className="w-5 h-5 mr-2 text-red-500" />
                        Low Stock Items
                        <span className="ml-auto bg-red-100 text-red-800 py-0.5 px-2.5 rounded-full text-xs font-medium">
                            &lt; 10 units
                        </span>
                    </h3>
                    <div className="flex-1 overflow-auto -mx-2 px-2 pb-4">
                        {analytics.lowStock.length === 0 ? (
                            <div className="h-full flex flex-col items-center justify-center text-center">
                                <div className="w-16 h-16 bg-green-50 rounded-full flex items-center justify-center mb-3 shadow-[inset_0_2px_4px_rgba(34,197,94,0.1)]">
                            ...
                            <div className="space-y-3">
                                {analytics.lowStock.map(p => (
                                    <div key={p.id} className="p-4 bg-red-50 border border-red-100 rounded-xl hover:shadow-md transition-shadow">
                                        <div className="flex justify-between items-start">
                                            <div>
                                                <h4 className="font-semibold text-gray-900 group-hover:text-red-700 transition-colors">{p.name}</h4>
                                                <div className="text-xs text-gray-500 mt-1 flex items-center space-x-2">
                                                    <span className="bg-white px-2 py-0.5 rounded border mr-2 font-mono shadow-sm">{p.product_code}</span>
                                                    {p.category && <span>{p.category}</span>}
                                                </div>
                                            </div>
                                            <div className="flex flex-col items-end">
                                                <span className="inline-flex items-center px-2.5 py-1 rounded-md text-sm font-bold bg-white border border-red-200 text-red-700 shadow-sm">
                                                    {p.stock_qty} left
                                                </span>
                                            </div>
                                        </div>
